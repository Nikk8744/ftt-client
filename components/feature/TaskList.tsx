...
const TaskList: React.FC<TaskListProps> = ({ tasks, projectId, isProjectOwner }) => {
  ...
  // Function to check if user can edit/delete a task
  const canModifyTask = (task: Task) => {
    return isProjectOwner || task.assignedUserId === user?.id;
  };
                ...
                {canModifyTask(task) && (
                  <>
                    ...
                    <Button 
                      ...
                      className="flex-1 lg:flex-none justify-center hover:bg-red-800 hover:border-red-300 transition-colors text-xs px-2 py-1"
                    >
                      <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
